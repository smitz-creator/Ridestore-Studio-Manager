...
const ALLOWED_USERS = ["Oskar", "Agnes"];
...
export default function Naming() {
  ...
  const isAllowed = ALLOWED_USERS.includes(user?.name ?? "");

  React.useEffect(() => {
    if (user && !ALLOWED_USERS.includes(user.name)) navigate("/");
  }, [user, navigate]);

  const { data: allProducts = [], isLoading } = useQuery<Product[]>({
    queryKey: ["products", "naming"],
    queryFn: () => api.getProducts({ uploadStatus: "post_production_done" }),
  ...
  const namedCount = completedIds.size;
  ...
  React.useEffect(() => {
    ...
    if (newCoIds.length > 0) {
      ...
      api.bulkUpdateStatus(newCoIds, "ready_for_upload").then(() => {
        qc.invalidateQueries({ queryKey: ["products"] });
      });
    }
  }, [allProducts, qc]);
  ...
  const toggleProduct = async (id: number) => {
    ...
    try {
      await bulkUpdateMut.mutateAsync({ productIds: [id], uploadStatus: "ready_for_upload" });
      toast({ title: "Product moved to Ready for Upload" });
    } catch {
      setCompletedIds(prev => { const n = new Set(prev); n.delete(id); return n; });
    }
  };
  ...
  const completeSession = async (products: Product[]) => {
    ...
    try {
      await bulkUpdateMut.mutateAsync({ productIds: ids, uploadStatus: "ready_for_upload" });
      toast({ title: `${ids.length} product${ids.length !== 1 ? "s" : ""} moved to Ready for Upload` });
    } catch {
      setCompletedIds(prev => {
        const n = new Set(prev);
  ...
  return (
    <Layout>
      <div className="max-w-4xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Naming</h1>
          <p className="text-muted-foreground text-sm mt-1">Rename files and verify all required assets are present</p>
        </div>

        <div className="bg-card border rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium">Overall Progress</span>
            <span className="text-sm font-semibold">{namedCount}/{totalProducts} products named</span>
          </div>
          <div className="w-full bg-zinc-700 rounded-full h-2.5">
            <div
              className="bg-yellow-500 h-2.5 rounded-full transition-all duration-300"
              style={{ width: totalProducts > 0 ? `${(namedCount / totalProducts) * 100}%` : "0%" }}
            />
          </div>
        </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {session.products.length} product{session.products.length !== 1 ? "s" : ""} · {doneInSession}/{session.products.length} named
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  {!allDone && (
                    <button
                      ...
                      className="px-3 py-1.5 text-xs font-medium bg-yellow-600 text-white rounded-md hover:bg-yellow-700 transition-colors disabled:opacity-50 whitespace-nowrap"
                    >
                      Complete Session
                    </button>
                  )}
                ...
                <div className="border-t divide-y">
                  {session.products.map(p => {
                    ...
                    return (
                      <div
                        key={p.id}
                        className={`flex items-center gap-4 px-4 py-3 transition-colors ${isDone ? "bg-yellow-900/20 opacity-60" : "hover:bg-secondary/20"}`}
                      >
                        <button
                          ...
                          className={`w-7 h-7 rounded-md flex items-center justify-center border-2 transition-all flex-shrink-0 ${
                            isDone
                              ...
                              : "border-zinc-600 hover:border-yellow-500 text-transparent hover:text-yellow-500"
                          }`}
                        >
