...
export default function Selection() {
  ...
  const { data: allProducts = [], isLoading } = useQuery<Product[]>({
    ...
    queryFn: () => api.getProducts({ uploadStatus: "ready_for_selection" }),
    refetchInterval: 5000,
    enabled: user?.name === "Philip",
  });
  ...
  const toggleProduct = async (id: number) => {
    ...
    try {
      ...
      toast({ title: "Product moved to Ready for Retouch" });
    } catch {
      setCompletedIds(prev => { const n = new Set(prev); n.delete(id); return n; });
    }
  };
  ...
  const completeSession = async (products: Product[]) => {
    ...
    try {
      await bulkUpdateMut.mutateAsync({ productIds: ids, uploadStatus: "ready_for_retouch" });
      toast({ title: `${ids.length} product${ids.length !== 1 ? "s" : ""} moved to Ready for Retouch` });
    } catch {
      setCompletedIds(prev => {
        const n = new Set(prev);
  ...
  if (user?.name !== "Philip") return null;

  return (
    <Layout>
      <div className="max-w-4xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Selection</h1>
          <p className="text-muted-foreground text-sm mt-1">Review sessions and mark products as selected</p>
        </div>

        <div className="bg-card border rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium">Overall Progress</span>
            <span className="text-sm font-semibold">{selectedCount}/{totalProducts} products selected</span>
          </div>
        ...
        {!isLoading && sessions.length === 0 && (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-sm">No products waiting for selection.</p>
          </div>
        )}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {session.products.length} product{session.products.length !== 1 ? "s" : ""} · {doneInSession}/{session.products.length} selected
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
